...
Hi Mark, the log transformations are here:
```{r}

new_col_labels <- paste(names(data.train[,c(11:13,15:21)]), "_log")
transformation_df <- log(data.train[,c(11:13,15:21)])
names(transformation_df) <- new_col_labels
data.train<-data.frame(data.train, transformation_df)
...
regfit.bwd = regsubsets(damt~. -part -ID, data=data.train, nvmax=30, method = "backward")
summary(regfit.bwd)
...
val.errors=rep(NA,30) #create a numbered list of x NA's
for(i in 1:30){
...
# This is the test MSE.  It shows that the one with 19 variables has the lowest MSE, but it kind of seems like we would be no worse off by choosing the model with only 9 variables.
val.errors
